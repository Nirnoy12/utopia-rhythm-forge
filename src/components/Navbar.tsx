...
const Navbar = () => {
  ...
  return (
    <>
      <nav className="fixed top-0 left-0 right-0 z-50 px-6 md:px-12 py-4 flex items-center justify-between backdrop-blur-md bg-background/60 border-b border-border/30">
        ...
        <div className="hidden md:flex items-center gap-1">
          {links.map((link) => (
            <MagneticButton key={link.to} strength={0.2}>
              <Link
                to={link.to}
                className={`px-4 py-2 text-sm font-sans uppercase tracking-widest transition-colors duration-300 ${
